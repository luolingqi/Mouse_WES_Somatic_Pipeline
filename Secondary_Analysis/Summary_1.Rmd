...
for(i in 1:nrow(items)) {
  ...
  file <- items[i,4]
  if ( file.info(file.path(path, file))$size >0 ) {
    ...
    assign(paste0("p_",cat), ggplot(tmp_df, aes(x=MAF)) + geom_histogram(color="black", fill="white", binwidth=0.01) + ylim(0,ylim) + ggtitle(gsub("\\.","\n",cat)) + theme(plot.title = element_text(size=16,face="bold",hjust = 0.5)))
    p <- get(paste0("p_",cat))
    print(p)
...
for(i in 1:nrow(items)) {
    ...
    if ( file.info(file.path(path, file))$size >0 )
    {
      tmp_df <- read.csv2(file = file.path(path, file), 
